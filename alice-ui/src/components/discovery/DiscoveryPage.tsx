  ...
  return <div className={`discovery-page discovery-page--${theme}`}>
    ...
    <main className="three-column">
      <aside className={`left-rail ${mobileRail === "left" ? "rail-open" : ""}`}><button className="drawer-close" onClick={() => setMobileRail(null)}><X /></button>
        <Card title="DISCOVERY PROGRESS"><div className="progress-ring"><span>42<small>%</small></span></div><div className="progress-copy"><b>Building Your Profile</b><span>Strong Foundation</span></div><ol className="steps">{[["Discover","Conversations & Insights"],["Understand","Patterns & Themes"],["Shape","Your Profile"],["Confirm","You're in Control"],["Connect","When You're Ready"]].map((s,i)=><li className={i===0?"active":""} key={s[0]}><i>{i===0?<Sparkles/>:<span>{i+1}</span>}</i><b>{s[0]}<small>{s[1]}</small></b></li>)}</ol></Card>
      ...
      <section className="center-panel"><div className="discovery-console"><div className="alice-identity"><AliceAvatar status={session.status} size="lg"/><h1>Alice <Sparkles/></h1><b>Your Discovery Guide</b><button className={`voice-mode-button ${session.voiceOn?"active":""}`} onClick={toggleVoice} aria-pressed={session.voiceOn}><Volume2/> Voice {session.voiceOn?"On":"Off"}</button><span className={`alice-debug alice-debug--${session.status}`}>Alice status: {session.status}{showPromptProfileSelector ? ` · ${activePromptProfile.name}` : ""}</span>{showPromptProfileSelector && <label className="prompt-mode-control">Alice Prompt Mode<select value={promptProfileId} onChange={event=>selectPromptProfile(event.target.value as AlicePromptProfileId)}>{alicePromptProfiles.map(profile=><option key={profile.id} value={profile.id}>{profile.name}</option>)}</select></label>}</div>
        ...
        <div className="transcript-review"><div><b>Your Transcript <span>(Review before sending)</span></b><label><FileText/> Edit</label></div><textarea value={review} onChange={e=>setReview(e.target.value)} placeholder="Your recorded transcript will appear here for review."/><button onClick={sendReview} disabled={!review.trim()}><Send/> Send to Alice</button></div>
        ...
        <div className="chat"><div ref={chatTop}/>{visibleTurns.map(turn => <div className={`message-row message-row--${turn.role}`} key={turn.id}>{turn.role === "alice" && <AliceAvatar status={turn.aliceStatusAtTime} size="sm"/>}<div className="bubble"><time>{turn.timestamp}</time><p>{turn.text}</p>{turn.role === "alice" && turn.aliceVoiceEnabled && <Volume2 className="bubble-speaker"/>}</div>{turn.role === "participant" && <span className="user-dot">E</span>}</div>)}</div>
      </section>
      <aside className={`right-rail ${mobileRail === "right" ? "rail-open" : ""}`}><button className="drawer-close" onClick={() => setMobileRail(null)}><X /></button>
        ...
        <Card title="PRIVACY & TRUST"><Info icon={<ShieldCheck/>} title="Your data is yours." text="We never sell your information."/><Info icon={<ShieldCheck/>} title="Private by default." text="You choose what to share."/><Info icon={<ShieldCheck/>} title="Secure & encrypted." text="Your privacy is protected."/><PlaceholderButton onClick={()=>openPlaceholder("Privacy details are coming soon.")}>Learn More</PlaceholderButton></Card>
        <Card title="QUICK ACTIONS"><div className="quick-actions"><button onClick={()=>openPlaceholder("Insights review is coming soon.")}><FileText/>Review My Insights</button><button onClick={()=>openPlaceholder("Preferences are coming soon.")}><Check/>Update My Preferences</button><button onClick={exportTranscript}><Download/>Export My Transcript</button><button onClick={()=>setModal("delete")}><Trash2/>Delete My Data</button></div></Card>
      </aside>
    </main>
