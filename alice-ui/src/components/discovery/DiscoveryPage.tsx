...
import { Bell, Check, ChevronDown, Compass, Copy, Download, FileText, Lightbulb, LogOut, Menu, Mic, Moon, Paperclip, Pause, Play, RefreshCcw, Send, ShieldCheck, Sparkles, Speaker, Square, Sun, Trash2, Volume2, X, Zap } from "lucide-react";
...
export default function DiscoveryPage({ onRestart }: { onRestart: () => void }) {
  ...
  // Nudges the participant to start responding at all when it's their turn
  // and nothing has been drafted yet -- the Speak tab's record button is the
  // one clear "next action" button in that state, so it's the one that
  // pulses (typing has no equivalent single button to highlight; the
  // textarea itself is the affordance there).
  const [startReminder, setStartReminder] = useState(false);
  useEffect(() => {
    setStartReminder(false);
    if (tab !== "speak" || recording || review.trim() || session.status !== "listening") return;
  ...
  const schemaCoverage: OzSchemaCoverageReport = useMemo(
    () => computeOzSchemaCoverage(session.ozCapture, participantTurnCount),
    [session.ozCapture, participantTurnCount]
  );
  ...
  return <div className={`discovery-page discovery-page--${theme}`}>
    ...
    <header className="topbar"><button className="icon-button mobile-menu" onClick={() => setMobileRail((current) => current === "left" ? null : "left")} aria-label={mobileRail === "left" ? "Close progress menu" : "Open progress menu"}><Menu /></button><a className="brand" href={LIGHTHOUSE_INFO_SITE_URL} target="_blank" rel="noopener noreferrer" title="Visit the Lighthouse informational site"><img className="brand-logo-img" src="/lighthouse-logo-icon.png" alt="Project Lighthouse"/></a><div className="page-title"><strong>LIGHTHOUSE DISCOVERY ENGINE</strong><small>You guide the conversation. <b>Alice</b> helps you be fully seen.</small></div><div className="top-actions" data-tour="top-actions"><span className="session-pill" data-tour="brand-status"><small>Session Status</small><b>● &nbsp; In Progress</b></span><button className="top-action-button tour-button" onClick={startGuidedTour}><Compass/> <span>Take the Tour</span></button><button className="top-action-button icon-button theme-toggle" onClick={toggleTheme} aria-label={`Use ${theme === "dark" ? "light" : "dark"} theme`}>{theme === "dark" ? <Sun/> : <Moon/>}</button><button className="top-action-button icon-button" aria-label="Notifications"><Bell /></button><span style={{ position: "relative" }}><button className="top-action-button profile-button" onClick={() => setProfileMenuOpen(v => !v)} aria-label={discoveryIdentity?.name ? `Open profile menu for ${discoveryIdentity.name}` : "Open profile menu"} aria-expanded={profileMenuOpen} title={discoveryIdentity?.name}><span>{discoveryIdentity?.name?.trim()?.[0]?.toUpperCase() ?? "?"}</span><ChevronDown/></button>{profileMenuOpen && <div className="profile-menu" style={{ position: "absolute", right: 0, top: "calc(100% + 6px)", borderRadius: "8px", boxShadow: "0 8px 24px rgba(24,39,75,.12)", minWidth: "160px", padding: "6px", zIndex: 30 }}>{discoveryIdentity?.name && <div className="profile-menu__identity" style={{ padding: "6px 8px", fontSize: "0.78rem", marginBottom: "4px" }}>{discoveryIdentity.name}<br/><span style={{ fontSize: "0.72rem" }}>{discoveryIdentity.email}</span></div>}<button className="profile-menu__signout" onClick={() => { setProfileMenuOpen(false); void handleSignOut(); }} style={{ width: "100%", textAlign: "left", border: "none", padding: "8px", borderRadius: "6px", cursor: "pointer", fontSize: "0.82rem" }}>Sign out</button></div>}</span></div><button className="icon-button mobile-menu" onClick={() => setMobileRail((current) => current === "right" ? null : "right")} aria-label={mobileRail === "right" ? "Close insights menu" : "Open insights menu"}><Sparkles /></button></header>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "6px", flex: 1, minWidth: "240px" }}>
              {schemaCoverage.fields.map(entry => (
                <div key={entry.field} style={{ display: "flex", justifyContent: "space-between", gap: "8px", padding: "6px 10px", borderRadius: "8px", border: "1px solid rgba(148,163,184,0.18)", fontSize: "0.78rem" }}>
                  <span>{OZ_SCHEMA_AREA_LABELS[entry.field]}</span>
