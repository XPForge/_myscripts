  ...
  return <div className={`discovery-page discovery-page--${theme}`}>
    ...
    <main className="three-column">
      <aside className={`left-rail ${mobileRail === "left" ? "rail-open" : ""}`}><button className="drawer-close" onClick={() => setMobileRail(null)}><X /></button>
        ...
        <Card title="TALKING WITH ALICE"><Info icon={<Mic/>} title="Press to speak" text="Tap the mic, say your answer, tap again when done."/><Info icon={<FileText/>} title="Review before sending" text="Your words appear as text — fix anything that's off."/><Info icon={<Send/>} title="Send when ready" text="Nothing goes to Alice until you press send."/></Card>
      </aside>
