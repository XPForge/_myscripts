...
export default function DiscoveryPage({ onRestart }: { onRestart: () => void }) {
  ...
  const [modal, setModal] = useState<"transcript" | "placeholder" | "delete" | "reset-profile" | "review" | null>(null); const [placeholder, setPlaceholder] = useState(""); const [mobileRail, setMobileRail] = useState<"left" | "right" | null>(null); const [sessionPaused, setSessionPaused] = useState(false);
  ...
  return <div className={`discovery-page discovery-page--${theme}`}>
    <header className="topbar"><button className="icon-button mobile-menu" onClick={() => setMobileRail("left")} aria-label="Open progress menu"><Menu /></button><div className="brand"><img className="brand-medallion" src="/lighthouse-medallion.png" alt="Lighthouse"/><span>LIGHTHOUSE<small>Human Insight. Real Connection.</small></span></div><div className="page-title"><strong>LIGHTHOUSE DISCOVERY ENGINE</strong><small>You guide the conversation. <b>Alice</b> helps you be fully seen.</small></div><div className="top-actions" data-tour="top-actions"><span className="session-pill" data-tour="brand-status"><small>Session Status</small><b>● &nbsp; In Progress</b></span><button className="top-action-button save-exit-button" onClick={() => { session.save(); openPlaceholder("Progress saved on this device."); }}>Save & Exit</button><button className="top-action-button" onClick={startGuidedTour}>Take the Tour</button><button className="top-action-button icon-button theme-toggle" onClick={toggleTheme} aria-label={`Use ${theme === "dark" ? "light" : "dark"} theme`}>{theme === "dark" ? <Sun/> : <Moon/>}</button><button className="top-action-button icon-button" aria-label="Notifications"><Bell /></button><button className="top-action-button profile-button" aria-label="Open profile menu"><span>E</span><ChevronDown/></button></div><button className="icon-button mobile-menu" onClick={() => setMobileRail("right")} aria-label="Open insights menu"><Sparkles /></button></header>
