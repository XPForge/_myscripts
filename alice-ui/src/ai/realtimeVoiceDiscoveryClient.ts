import type { RealtimeDiscoverySession } from "./lighthouseDiscoveryService";
...
export async function startRealtimeVoiceDiscovery(
  realtimeSession: RealtimeDiscoverySession,
  ...
  status("Realtime voice discovery is active. Speak naturally now.");
  ...
  const stop = async () => {
    ...
    status("Realtime voice discovery session stopped.");
  };
